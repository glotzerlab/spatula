...
#include <pybind11/pybind11.h> // For PYBIND11_OVERRIDE_PURE and PYBIND11_OVERRIDE
...
class PyOptimizer : public Optimizer {
    ...
    /// Get the next point to compute the objective for.
    void internal_next_point() override { PYBIND11_OVERRIDE_PURE(void, Optimizer, internal_next_point); }
    /// Record the objective function's value for the last querried point.
    void record_objective(double objective) override { PYBIND11_OVERRIDE(void, Optimizer, record_objective, objective); }
    /// Returns whether or not convergence or termination conditions have been met.
    bool terminate() const override { PYBIND11_OVERRIDE_PURE(bool, Optimizer, terminate); }

    /// Create a clone of this optimizer
    virtual std::unique_ptr<Optimizer> clone() const override { return std::make_unique<PyOptimizer>(*this); }
};
