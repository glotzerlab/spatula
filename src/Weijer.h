...
/**
 * @brief Given WignerD matrices and spherical harmonic expansion coefficients \f$ Q_{m}^{l} \f$
 * compute all symmetrized expansion's coefficients using symmetrize_qlm.
 *
 ...
 * @param D_ij The vector of all WignerD matrices for given symmetries or point groups.
 * @param sym_qlm_buf The vector to place the symmetrized expansion coefficients into. The size of
 * the overall vector is expected to be the same as D_ij, but the capacity or size of each element
 * of sym_qlm_buf does not need to be, having the proper capacity will improve performance.
